...
floaty.makeFloaty = function(element, options){
  ...
  if (options){
    ...
    if (options.onclick){
      floater.onclick = options.onclick;
    }
  }

  floater.addEventListener('mouseover', floaty.makeMouseoverCallback(floater)); // 鼠標懸停時

  floater.addEventListener('mousedown', floaty.makeMousedownCallback(floater)); // 按下鼠標左鍵時

  floater.addEventListener('touchstart', floaty.makeTouchstartCallback(floater), false); //觸摸元素時

  floater.addEventListener('mouseup', floaty.makeMouseupCallback(floater)); //釋放鼠標左鍵時

  floater.addEventListener('touchend', floaty.makeTouchEndCallback(floater), false); //從元素移除手指時

  floater.addEventListener('mousemove', floaty.makeMousemoveCallback(floater)); //在元素內移動時

  floater.addEventListener('Touchmove', floaty.makeTouchmoveCallback(floater), false); //移動手指時

  // floater.addEventListener('tap', floaty.maketapCallback(floater), false); //手指點擊時
};
...
floaty.makeMouseoverCallback = function(floater){ // 鼠標懸停時
  return function(){
...
floaty.makeMousedownCallback = function(floater){ // 按下鼠標左鍵時
  return function(){
...
floaty.makeTouchstartCallback = function(floater){ //觸摸元素時
  return function(){
    floater.mouse_clicked = true;
    floater.activate = true;
    floater.addClass('active');
...
floaty.makeMouseupCallback = function(floater){ //釋放鼠標左鍵時
  return function(){
...
floaty.makeTouchEndCallback = function(floater) { //從元素移除手指時
  return function() {
    floater.mouse_clicked = false;
    floater.removeClass('active');
    if (floater.activate) {
...
floaty.makeMousemoveCallback = function(floater){ //在元素內移動時
  return function(e){
    e.preventDefault();
...
floaty.makeTouchmoveCallback = function(floater){ //移動手指時
  return function(e){
    e.preventDefault();
    if (floater.mouse_clicked) {
